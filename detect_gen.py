...
def main(args):
    ...
    if args.multi:
        ...
        args.cls_num = 2 
    # process path
    model_path = 'ckts_exp_gen_{}'.format(args.exp_name)
    os.popen('hdfs dfs -mkdir -p hdfs://deepfake_train/{}'.format(model_path))
    if not os.path.exists(model_path):
        os.mkdir(model_path)

    log_path = 'summary_exp_gen/{}'.format(args.exp_name)
    ...
    model_gen_fake = kernels.pixelKernelGenerator(n_blocks=args.gen_blocks, radius=args.radius, norm_layer=args.gen_norm_type, output_nc=args.kernel_channels, norm=args.norm, learn_norm=args.learn_norm, sigmoid=args.sigmoid, no_clip=args.no_clamp)
    model_gen_real = kernels.pixelKernelGenerator(n_blocks=args.gen_blocks, radius=args.radius, norm_layer=args.gen_norm_type, output_nc=args.kernel_channels, norm=args.norm, learn_norm=args.learn_norm, sigmoid=args.sigmoid, no_clip=args.no_clamp)

    
    model_gen_real = apex.parallel.convert_syncbn_model(model_gen_real)
    model_gen_real = model_gen_real.cuda()  
    model_gen_fake = apex.parallel.convert_syncbn_model(model_gen_fake)
    ...
    optimizer_gen_real = radam.RAdam(model_gen_real.parameters(),lr=args.gen_lr,weight_decay=args.weight_decay)
    optimizer_gen_fake = radam.RAdam(model_gen_fake.parameters(),lr=args.gen_lr,weight_decay=args.weight_decay)

      
    [model, model_gen_real, model_gen_fake], [optimizer, optimizer_gen_real, optimizer_gen_fake] = amp.initialize(
        [model, model_gen_real, model_gen_fake], [optimizer, optimizer_gen_real, optimizer_gen_fake], num_losses = 3,
                                    opt_level="O0",
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        utils.adjust_learning_rate_gen([optimizer_gen_real], args, epoch, args.lr_decay_factor, args.lr_decay_epochs)
        utils.adjust_learning_rate_gen([optimizer_gen_fake], args, epoch, args.lr_decay_factor, args.lr_decay_epochs)
        train([dataloader_real, dataloader_fake], criterion, [model, model_gen_real, model_gen_fake], [optimizer, optimizer_gen_real, optimizer_gen_fake], epoch, args.log_interval, args.segment_length, val_dataloader=val_dataloader, args=args)
        if (epoch + 1) % args.snapshot_interval == 0:
            print('===> Saving models of epoch {}...'.format(epoch+1))
            state = {'state': model.state_dict(),'epoch': epoch+1}
            state_gen = {'state_real': model_gen_real.state_dict(), 'state_fake': model_gen_fake.state_dict(), 'epoch': epoch}
            torch.save(state_gen, os.path.join(model_path, 'gen_{}.pt'.format(epoch+1)))
            a = os.path.join(model_path, 'gen_{}.pt'.format(epoch+1))
    ...
    for i, ((inputs_real, targets_real), (inputs_fake, targets_fake)) in enumerate(zip(train_loader_real, train_loaders_fake)):
        ...
        if args.gen_train_type == 'pert':
            for g_num in range(args.g_per_d):
                ...
                pert_real = inputs_real_grad / inputs_real_grad_norm * new_norm
                generations_real = pert_real + inputs_real
                generations_real = torch.clamp(generations_real, -1, 1)
               
                if (i+1) % args.hist_interval ==0:
                    writer.add_histogram('pert_real', pert_real.detach(), global_step=global_step)
                ...
                # genloss_att_real = -criterion(pred_real, targets_aug_real).mean()
                genloss_att_fake = criterion(pred_fake, 1-targets_aug_fake).mean()
                inputs_fake_grad = torch.autograd.grad(genloss_att_fake, inputs_fake_tmp)[0] 
                inputs_fake_grad_norm = torch.norm(inputs_fake_grad, p=2., dim=(1,2,3), keepdim=True)
                pert_fake = inputs_fake_grad / inputs_fake_grad_norm * new_norm
                generations_fake = pert_fake + inputs_fake
                generations_fake = torch.clamp(generations_fake, -1, 1)

                if  (i+1) % args.hist_interval ==0:
                    writer.add_histogram('pert_fake', pert_fake.detach(), global_step=global_step)
            ...
            for g_num in range(args.g_per_d):
                ...
                if args.xception:
                    sigmas_real = sigmas_real[:, :, :299, :299]
                ...
                with amp.scale_loss(genloss_real, optimizer_gen_real, loss_id=0) as scaled_genloss:
                    scaled_genloss.backward()
                optimizer_gen_real.step()
                if (i+1) % args.hist_interval ==0:
                    writer.add_histogram('sigmas_real', sigmas_real.detach(), global_step=global_step)
                    tmp = list(model_gen_real.parameters())[0]
                    writer.add_histogram('real first layer weight', tmp.detach(), global_step=global_step)
                    tmp = list(model_gen_real.parameters())[-1]
                    writer.add_histogram('real last layer weight', tmp.detach(), global_step=global_step)
                    if args.learn_norm:
                        writer.add_scalar('real norm', model_gen_real.module.new_norm.data.detach().squeeze(), global_step=global_step)

                # fake model
                optimizer_gen_fake.zero_grad()
                sigmas_fake = model_gen_fake(inputs_fake)
                if args.xception:
                    sigmas_fake = sigmas_fake[:, :, :299, :299]
                ...
                with amp.scale_loss(genloss_fake, optimizer_gen_fake, loss_id=1) as scaled_genloss:
                    scaled_genloss.backward()
                optimizer_gen_fake.step()
                if  (i+1) % args.hist_interval ==0:
                    writer.add_histogram('sigmas_fake', sigmas_fake.detach(), global_step=global_step)
                    tmp = list(model_gen_fake.parameters())[0]
                    writer.add_histogram('fake first layer weight', tmp.detach(), global_step=global_step)
                    tmp = list(model_gen_fake.parameters())[-1]
                    writer.add_histogram('fake last layer weight', tmp.detach(), global_step=global_step)
                    if args.learn_norm:
                        writer.add_scalar('fake norm', model_gen_fake.module.new_norm.data.detach().squeeze(), global_step=global_step)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--weight', action='store_true',
        help='Use weighted version loss')
    ...
    parser.add_argument('--learn_norm', action='store_true',
        help='Use learnable norm')
    parser.add_argument('--sigmoid', action='store_true',
        help='Use sigmoid other than abs as sigma')
    parser.add_argument('--tanh', action='store_true',
        help='Use tanh for flow generator')

    parser.add_argument('--clamp_res', action='store_true',
        help='Clamp res image') 
    parser.add_argument('--fake_set', action='store_true',
        help='Use fake_set')
    parser.add_argument('--real_set', action='store_true',
        help='Use real_set')
    parser.add_argument('--no_res', action='store_true',
        help='Dataset gen not use res')
    ...
    parser.add_argument('--kernel_channels', type=int,
        help='channel num for kernels gen model .', default=1)
    parser.add_argument('--radius', type=int,
        help='radius of gaussian kernel.', default=3)
    parser.add_argument('--pretrain_g', action='store_true',
        help='Use our pretrained model')
    parser.add_argument('--gen_norm_type', type=str,
        help='type of norm in model-gen', default='bn')
    ...
    parser.add_argument('--gen_blocks', type=int, default=3,
        help="# of res blocks in generator")
    parser.add_argument('--gen_lr', type=float,
        help='Initial model gen learning rate.', default=2e-3)


    parser.add_argument('--max_epoch', type=int,
        help='Number of training epoches.', default=20)
